//for storing cell properties
// in matrix form

let collectedSheetDB = [];  //Contains all SheetDB

let sheetDB = []; //onr grid db

{
    let addSheetBtn = document.querySelector(".sheet-add-icon");
    addSheetBtn.click();
}
//ye object hmne  sheetHandlig m define krdiya hai


// for (let i = 0; i < rows; i++) {
//     let sheetRow = []; //each each has 26 objects
//     for (let j = 0; j < cols; j++) {
//         let cellProp = { //each cell prop is stored in an object
//             bold: false,
//             italic: false,
//             underline: false,
//             alignment: "left",
...
//             children: [],
//         }
//         sheetRow.push(cellProp); //adds object to row array
//     }
//     sheetDB.push(sheetRow); //adds row to the sheet array
// }
...
cell.addEventListener("click", (e) => {
 ...
 function getCellAndCellProp(address) {
    ...
    let cell = document.querySelector(`.cell[rid="${rid}"][cid="${cid}"]`);//UI, .cell[][] : these sqaure brackets are used to access attributes .bcz rid and cid are stored as attribytes of div : cell
    let cellProp = sheetDB[rid][cid];//Database
    return [cell, cellProp];
}
                
function decodeRIDCIDFromAddress(address) {
                // address -> "A1" , if column number is like "GA2",
                //  then this has to chenge a bit
